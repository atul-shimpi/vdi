...
class XFilter
  ...
  # Defines a filtering that checks for in-equality.
  #
  # Example:
  #   @filter = Filter.new
  #   @filter.not_equal params[:id], 'an_id'
  def not_equal(key, filtered)
    key = key.to_s
    @filters << ["(#{filtered} != ? or #{filtered} is NULL)", key] unless key.nil? || key.empty?
  end
  
  # Defines a filtering that checks for not null.
  #
  # Example:
  #   @filter = Filter.new
  #   @filter.is_not_null 'an_id'
  def is_not_null(filtered)
    @filters << ["(#{filtered} is NOT NULL and #{filtered}!='')"]
  end
  
    # Defines a filtering that checks for null.
  #
  # Example:
  #   @filter = Filter.new
  #   @filter.is_null 'an_id'
  def is_null(filtered)
    @filters << ["(#{filtered} is NULL or #{filtered} = '')"]
  end
  ...
  # Defines a filtering that checks for string like
  #
  # Example:
  #   @filter = Filter.new 
  #   @filter.like :other_name, 'name'
  def like(key, filtered)
    key = key.to_s  
    @filters << ["#{filtered} like ?", '%'+key+'%'] unless key.nil? || key.empty?
  end

  # Defines a filtering that checks for in
  #
  # Example:
  #   @filter = Filter.new 
  #   @filter.in :ids, 'id'
  def in_set(key, filtered)
    @filters << ["#{filtered} in (#{key})"] unless key.nil? || key.empty?
  end
  
  def more_condition(condition)
    @filters << ["#{condition}"] unless condition.nil? || condition.empty?
  end
  
  def or(condition1, condition2)
    @filters << [" (#{condition1} OR #{condition2}) "]
  end
