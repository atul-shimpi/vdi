...
@@schedule3_name = @@config["task_checker_file"] + "_3"
aFile = File.new(@@schedule3_name,"r")
...
if (nowTime >= lastTime && nowTime < (lastTime + 2)) ||
   ((nowTime + 24 * 60) >= lastTime && (nowTime + 24 * 60) < (lastTime + 2))
    puts "The schedule3 works well: " + currentTime.to_s
else
    puts "The schedule3 is dead at: " +  currentTime.to_s
#   Remove the mysql operation since it's has been moved to antoher machine    
#   `/etc/init.d/mysqld restart`
#    `/etc/init.d/httpd restart`
    `/usr/local/bin/ruby /var/www/gdevvdi/script/schedule3.rb stop`
    `/usr/local/bin/ruby /var/www/gdevvdi/script/schedule3.rb start`
end