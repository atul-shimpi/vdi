...
@@schedule4_check_file = @@config["task_checker_file"] + "_4"
@@deploy_count = 0
@@recover_count = 0

def deploy_job(job)
  ScheduleTask.wtire_current_task_time(@@schedule4_check_file)
...
def create_instances_task
  ...
  ScheduleTask.each_job("('Pending')") do |index, job, pending_jobs_count|
    if (index == 0) and (pending_jobs_count > 6)
...
Daemons.run_proc(
  ...
  begin
    ...
    @@scheduler.every("25s") do
      ...
      if ScheduleTask.jobs_count("('Deploying', 'Pending')").size == 0
        ScheduleTask.wtire_current_task_time(@@schedule4_check_file)
      else
