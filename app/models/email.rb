...
class Email < ActiveRecord::Base
  ...
  def self.mail_to_admin(subject, content)
    ...
    email.to = "ryan.shillington@devfactory.com, atul.shimpi@gteamstaff.com"
    email.content = content
    email.save     
  end
  ...
  def self.mail_to_admin_about_cost_warning_jobs(jobs)
    ...
    jobs.each do |job|
      email = Email.new
      email.subject = "[VDI 1] Job cost too much money -- " + job.id.to_s
  ...
  def self.send_low_cpu_usage_power_off_notification(job, cpu_usage)
    ...
    Specifically, this job has used an average of ' +  cpu_usage.to_s + '% CPU over the past hour.
    This job is being stopped because clearly nobody is using it.<br><br>
    Job name: <a href=https://vdi.gdev.com/jobs/' + job.id.to_s + '>' + job.name + '</a>, Created at: ' + job.created_at.to_s
    email.content = email.content + ", " + job.usage + "<br><br>"
    
    email.content = email.content + 'You can request for the exemption of your job from this policy by replying to this mail 
    the reason why this job needs exemption. If the reason is justifiable, your job will be exempted within 24 hours.<br>
    Kindly copy-paste below line in the subject line.<br>
    Exempt Job ' + job.id.to_s + ' from Low CPU Usage policy'
